...
const BUILTINS = module.exports = {
  ...
  alphanumeric(typeDef, typeArg, errorPrefix) {
  ...
  any(typeDef, typeArg, errorPrefix) {
    assert(_.isObject(typeDef), 'Type definition must be an object.');
    if (typeDef.maxSize || typeDef.minSize) {
      // Must serialize to measure length.
      const serData = JSON.stringify(typeArg);
      const serLength = serData ? serData.length : 0;
  ...
  base64_buffer(typeDef, typeArg, errorPrefix) {
    assert(_.isObject(typeDef), 'Type definition must be an object.');
    BUILTINS.string(typeDef, typeArg, errorPrefix);
  ...
  boolean(typeDef, typeArg, errorPrefix) {
  ...
  buffer(typeDef, typeArg, errorPrefix) {
    assert(_.isObject(typeDef), 'Type definition must be an object.');
  ...
  epoch_timestamp_ms(typeDef, typeArg, errorPrefix) {
    ...
    //
    const MIN_EPOCH_MS = 631152000000; // Mon, 01 Jan 1990 00:00:00 GMT
  ...
  factor(typeDef, typeArg, errorPrefix) {
    BUILTINS.string(typeDef, typeArg, errorPrefix);
    if (!_.find(typeDef.factors, (f) => { return f === typeArg; })) {
      throw ErrorMessage(errorPrefix, fmt(
  ...
  function(typeDef, typeArg, errorPrefix) {
  ...
  hex_buffer(typeDef, typeArg, errorPrefix) {
    // Multiply length constraints by 2 for a hex string.
    const td = _.cloneDeep(typeDef);
  ...
  integer(typeDef, typeArg, errorPrefix) {
    assert(_.isObject(typeDef), 'Type definition must be an object.');
  ...
  ip_address(typeDef, typeArg, errorPrefix) {
    assert(_.isObject(typeDef), 'Type definition must be an object.');
    BUILTINS.string(typeDef, typeArg, errorPrefix);
    const isIPv6 = typeArg.match(/^(?:[A-F0-9]{1,4}:){7}[A-F0-9]{1,4}$/i);
    const isIPv4 = typeArg.match(/^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/);
    if (!isIPv4 && !isIPv6) {
  ...
  literal(typeDef, typeArg, errorPrefix) {
    assert(_.isObject(typeDef), 'Type definition must be an object.');
  ...
  number(typeDef, typeArg, errorPrefix) {
    assert(_.isObject(typeDef), 'Type definition must be an object.');
  ...
  string(typeDef, typeArg, errorPrefix) {
    assert(_.isObject(typeDef), 'Type definition must be an object.');
