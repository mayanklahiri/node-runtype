...
function _getDefault(schema) {
  if (_.isFunction(schema.default)) return schema.default();
...
function _constructRecursive(schema, value, options, treePath, state) {
  ...
  if (schema.type in library) {
    const refType = library[schema.type];
    const clonedSchema = _.cloneDeep(schema);
    delete clonedSchema.type;
    schema = _.merge(refType, clonedSchema);
  }
  ...
  if (_.isUndefined(value)) {
    ...
    const defaultValue = _getDefault(schema);
    if (options.fillDefaults && !_.isUndefined(defaultValue)) {
      value = defaultValue;
    } else {
      // Not optional and no default fillable, this is a validation error.
  ...
  if (schema.type === 'array') {
    ...
    if (schema.elements) {
      ...
      if (schema.elements.length !== value.length) {
        state.errors.push(new RuntypeError({
          code: 'bad_value',
          message: `${pathStr}: array size is out of range.`,
          path: pathClone,
        }));
        return;
      }
