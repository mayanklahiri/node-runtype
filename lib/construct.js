...
function construct(typeDef, typeArg, errorPrefix) {
  ...
  if (types[typeDef.type]) {
    const refSchema = types[typeDef.type];
    const tdCopy = _.cloneDeep(typeDef);
    delete tdCopy.type;
  ...
  if (typeDef.type === 'object') {
    ...
    return _.mapValues(typeDef.fields, (fieldSchema, fieldName) => {
  ...
  if (typeDef.type === 'array') {
    ...
    if (typeDef.elements) {
      ...
      return _.map(typeDef.elements, (elemType, idx) => {
